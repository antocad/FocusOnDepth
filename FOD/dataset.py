...
class TestDataset(Dataset):
  def __init__(self, config, path_images, transform):
  ...
  def __getitem__(self, idx):
    if torch.is_tensor(idx):
        idx = idx.tolist()
    img = Image.open(self.paths_images[idx])
    img = self.transform(img)
    return img, self.paths_images[idx]
...
class AutoFocusDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if random.random() < self.p_rot:
            #rotate
            random_angle = random.random()*170 - 85 #[-85 ; 85]
